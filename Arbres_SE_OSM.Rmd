...
library(ggmap)# carto +
library(ggspatial) # carto + en train de remplacer ggmap ?
library(rosm)
...
st_write(arbres_se.final.shp, dsn = "arbres_se_final.geojson")
```
...
```{r}
arbres_xp <- arbres_se.final.shp[zone.shp,] # un clip sur la zone
st_write(arbres_se.final.shp, dsn = "arbres_se_xp.geojson")
summary(arbres_xp)
...
On regarde la répartition des genres 
...
ggmap(se_stan) +
    geom_sf(data = arbres_xp, inherit.aes = FALSE, pch = 16, alpha = 0.5, col = "green", size = 0.5) 
