...
arbres_se.final.shp <- arbres_se.shp %>% 
  ...
  select(-APIC_CDATE, -APIC_MDATE, -NATURE, -CD_LIMCO, -RIVOLI) # on enleve les champs non utiles dans OSM
...
```{r}
leaf <- read.csv2("data/leaft_type.csv", sep = "\t", stringsAsFactors = FALSE) # lecture de csv
leaf[leaf$leaf_cycle == "evergreen and deciduous",] <- "deciduous and evergreen" # verif si besoin de le faire pour le second 
head(leaf)
leaf2 <- read.csv2("data/leaf_type_ajout.csv", sep = ",", stringsAsFactors = FALSE) # ouverture du fichier complementaire
summary(leaf2)

leaf_tot <- rbind(leaf,leaf2) #on ajoute les deux
leaf_tot$Genus <- str_trim(leaf_tot$Genus) # on supprime des whitespaces
# sort(leaf_tot$Genus)
...
sort(unique(arbres_se.final.shp$genus)[unique(arbres_se.final.shp$genus) %in% leaf_tot$Genus]) # liste des genres presents
sort(unique(arbres_se.final.shp$genus)[!(unique(arbres_se.final.shp$genus) %in% leaf_tot$Genus)]) # liste des genres absents
```
