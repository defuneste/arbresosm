...
arbres_lyon_final.shp <- arbres_lyon.shp %>%
  ...
  select(-hauteurfut, -diametreco, -rayoncouro, -circonfere, -anneeplant, -architectu, -localisati, -naturereve, -surfacecad, 
         -mobilierur, -commune, -codefuv, -nomvoie, -identifian, -numero, -codegenre, -filtre, -espece, -gid)
names(arbres_lyon_final.shp)
```

#### rajout de Leaf_type et Leaf_cycle

```{r, import du csv d'après le wiki}
...
```{r}
sort(unique(arbres_lyon_final.shp$genre)[unique(arbres_lyon_final.shp$genus) %in% leaf$Genus]) # liste des genres presents
sort(unique(arbres_lyon_final.shp$genre)[!(unique(arbres_lyon_final.shp$genus) %in% leaf$Genus)]) # liste des genres absents
```
